...
export function ProductFormPage() {
  ...
  function validate(): boolean {
    ...
    if (form.serial_number.trim().length < 2) e.serial_number = 'Serial number minimal 2 karakter';
    if (!form.barcode.trim()) e.barcode = 'Barcode wajib diisi';
    if (form.base_price <= 0) e.base_price = 'Harga pokok harus lebih dari 0';
    if (form.available_stock < 0) e.available_stock = 'Stok tidak boleh negatif';
    setErrors(e);
    ...
    if (value === ADD_BRAND) {
      ...
      if (name && name.trim()) {
        ...
        if ('data' in result && result.data) {
          setForm((f) => ({ ...f, brand_id: result.data.brand_id }));
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if ('error' in result) {
      ...
      // Petakan error unik ke field yang relevan
      if (msg.includes('Barcode')) setErrors((p) => ({ ...p, barcode: msg }));
      else if (msg.includes('Serial')) setErrors((p) => ({ ...p, serial_number: msg }));
  ...
  return (
    <div>
      <div className={page.pageHead}>
        <h1 className={page.pageTitle}>{isEdit ? 'Edit Produk' : 'Tambah Produk'}</h1>
      </div>
      ...
      <form className={page.formStack} onSubmit={handleSubmit} noValidate>
        ...
        />
        <Button type="button" variant="ghost" onClick={() => setShowScanner((s) => !s)}>
